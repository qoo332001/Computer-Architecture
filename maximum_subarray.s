...
maxSubArray:
    ...
    sw t0,0(sp)     #push t0 register
    sw t1,4(sp)     #push t1 register
    sw t2,8(sp)     #push t2 register
    sw t3,12(sp)    #push t3 register
    sw t4,16(sp)    #push t4 register
 ...
 next:
    ...
    lw t0,0(sp)          #restore the saved register t0
    lw t1,4(sp)          #restore the saved register t1
    lw t2,8(sp)          #restore the saved register t2
    lw t3,12(sp)         #restore the saved register t3
    lw t4,16(sp)         #restore the saved register t4
